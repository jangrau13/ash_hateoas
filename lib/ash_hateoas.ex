defmodule AshHateoas do
  ...
  and actor. This package renders that result as Hydra — affordances become
  `hydra:Operation` nodes served as JSON-LD, so that clients which have never
  seen this code can act on them.

  Affordances are derived from what a resource already declares — its actions,
  its derived routes, its policies, and its `AshStateMachine` transitions where
  ...
  collection-level and cold-start case, where no state gate applies.
