...
| a public to-many relationship | on a node: a property keyed by the relationship name → `{"@id": …/:id/<name>, "@type": "Collection"}`; in the ontology: an `owl:ObjectProperty` + `hydra:Link` whose `rdfs:range` is the property's own **collection class** (not the member class — the value *is* the collection, so a member range would say the collection is one of its own members) |
