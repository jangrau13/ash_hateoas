...
| a public to-many relationship | on a node: a property keyed by the relationship name → a blank-node `hydra:Collection` carrying `hydra:member` when the action loaded it, and **omitted** when it did not (empty would assert "none"); in the ontology: an `owl:ObjectProperty` + `hydra:Link` whose `rdfs:range` is the property's own **collection class** (not the member class — the value *is* the collection, so a member range would say the collection is one of its own members) |
